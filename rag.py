...
TOP_K_KB = 3
TOP_K_USER = 2
LOW_SIMILARITY_THRESHOLD = 0.35  # text-embedding-3-small produces lower magnitudes than Gemini
...
class PetCareAdvisor:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        self._kb_chunks: list[tuple[str, str]] = []
    ...
    def _load_kb(self) -> None:
        ...
        self._kb_embeddings = self._embed(texts)
    ...
    def retrieve(
        ...
        if user_snippets:
            user_embeds = self._embed([s[1] for s in user_snippets])
    ...
    def _chat(self, system: str, user: str) -> str:
        """Call the chat completion API with one rate-limit retry."""
        try:
            ...
            log.warning("Rate-limited; sleeping 10s before retry")
            time.sleep(10)
            resp = self.client.chat.completions.create(
                model=GEN_MODEL,
                messages=[
    ...
    def _critique(self, question: str, context: str, draft: str) -> tuple[bool, str]:
        ...
        resp = self._chat(CRITIC_PROMPT, user_msg)
    ...
    def ask(self, question: str, owner: Owner | None = None) -> dict:
